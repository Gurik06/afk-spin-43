...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-f731fcc0";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-f731fcc0">'+'<span class="tile-face-f731fcc0 tile-front-f731fcc0"></span>'+'<span class="tile-face-f731fcc0 tile-back-f731fcc0"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-f731fcc0")){return;}
var back=tile.querySelector(".tile-back-f731fcc0");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-f731fcc0");if(isMine){tile.classList.add("boom-f731fcc0");}
if(dim){tile.classList.add("dimmed-f731fcc0");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-f731fcc0")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-f731fcc0");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-f731fcc0")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-f731fcc0");void panelEl.offsetWidth;panelEl.classList.add("jolt-f731fcc0");}
endRound();}
